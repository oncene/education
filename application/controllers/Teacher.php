...
class Teacher extends CI_Controller
{
    ...
    function forum($param1 = '', $param2 = '', $param3 = '') 
    {
        if ($param1 == 'create') 
        {
            ...
            foreach($students as $row)
            {
                $notify['notify'] = "<strong>".$this->crud_model->get_name('teacher', $this->session->userdata('login_user_id')).":</strong>,". get_phrase('added_new_forum_discussion');
                $notify['user_id'] = $row['student_id'];
                $notify['user_type'] = 'student';
                $notify['type'] = 'forum';
    ...
    function create_online_exam($info = '') 
    {
        ...
        $notifys['notify'] = "<strong>". $this->crud_model->get_name('teacher', $this->session->userdata('login_user_id'))."</strong> ". get_phrase('online_exam_notify')." <b>".$this->input->post('exam_title')."</b>";
